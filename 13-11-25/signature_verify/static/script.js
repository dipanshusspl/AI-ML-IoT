...
document.getElementById("capture").addEventListener("click", async () => {
  ...
  try {
    ...
    if (data.verified) {
      resultText.innerText = `✅ Signature Verified (similarity: ${data.similarity.toFixed(3)})`;
      resultText.style.color = "green";
    } else if (data.verified === false) {
      resultText.innerText = `❌ Verification Failed (similarity: ${data.similarity.toFixed(3)})`;
